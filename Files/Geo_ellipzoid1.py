# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 15:06:10 2017
...
solver.set_initial_value(x[0,:],t[0])
for ii in range(N):
    x[ii,:] = solver.integrate(t[ii])


# =============================================================================
# for i in np.arange(0, N):
#     x[i+1,:] = integrators.Trap(f, t[1]-t[0], x[i,:], t[i])
# =============================================================================

# visualize in local coordinates
plt.figure()
plt.plot(t, x[:,0], 'b', label = 'theta')
plt.plot(t, x[:,1], 'k', label = 'phi')
plt.legend(loc = 'best')
plt.xlabel('t')
plt.show()
...
X = np.zeros((3,N))
for i in np.arange(0, N):
    X[:,i] = patch(x[i,0], x[i,1])

fig = plt.figure()
ax = Axes3D(fig)
ax.plot(X[0,:],X[1,:],X[2,:], color= 'k')
